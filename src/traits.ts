...
export function handleTraitsAndTypesCreated(event: TraitsAndTypesCreatedEvent): void {
  ...
  for (let i = 0; i < traitCount; i++) {
    ...
    trait.traitType = concat2(event.address.toHexString(), event.params.traitTypeIndexes[i].toString());
    trait.save();
  };

  const traitTypeCount = event.params.traitTypeNames.length;
  for (let i = 0; i < traitTypeCount; i++) {
    let traitType = new TraitType(concat2(event.address.toHexString(), i.toString()));
